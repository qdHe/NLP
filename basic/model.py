    ...
    def _build_forward(self):
        ...
        with tf.variable_scope("main"):
            ...
            if config.dynamic_att:
                ...
                # _, (sf, sb) = bidirectional_dynamic_rnn(cell, cell, p0, x_len, dtype='float', scope='s')
                # print(sf)
                # print(sb)

                # [N, M, JX, d]
                (sf, sb), (sf2, sb2) = bidirectional_dynamic_rnn(cell, cell, p0, x_len, dtype='float', scope='s')

                # print(sf2.get_shape)

                s = tf.concat(3, [sf, sb])
                s = tf.slice(s, [0, M-1, JX-1, 0], [N, 1, 1, 2*d])
                # [N, 2d]
                s = tf.squeeze(s)

                sq = tf.tile(tf.expand_dims(s, 1), [1, JQ, 1]) # s tiled Q times
                sq = tf.reshape(sq, [-1, 2*d])
                u = tf.reshape(u, [-1, 2*d])
                qz = tf.tanh(_linear([sq, u], 2*d, bias=True, bias_start=0.0, scope='qz'))
                qf = tf.sigmoid(_linear([sq, u], 2*d, bias=True, bias_start=0.0, scope='qf'))
                print(qf.get_shape, u.get_shape, qz.get_shape)
                u2 = qf * u + (1 - qf) * qz
                u2 = tf.reshape(u2, [N, JQ, 2*d])


                sc = tf.tile(tf.expand_dims(tf.expand_dims(s, 1), 1), [1, M, JX, 1])
                sc = tf.reshape(sc, [-1, 2*d])
                h = tf.reshape(h, [-1, 2*d])
                cz = tf.tanh(linear([sc, h], 2*d, bias=True, bias_start=0.0, scope='cz'))
                cf = tf.sigmoid(linear([sc, h], 2*d, bias=True, bias_start=0.0, scope='cf'))
                h2 = cf * h + (1 - cf) * cz
                h2 = tf.reshape(h2, [N, M, JX, 2*d])

                p0 = attention_layer(config, self.is_train, h2, u2, h_mask=self.x_mask, u_mask=self.q_mask, scope="p1", tensor_dict=self.tensor_dict)
